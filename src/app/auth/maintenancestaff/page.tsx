...
import staffData from "../../../data/maintenancestaff_credentials.json";

export default function MaintenanceStaffAuthPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (isLogin) {
      const staffExists = staffData.staff.some(
        (staff) => staff.username === username && staff.password === password
      );
      if (staffExists) {
        router.push("/roles/maintenancestaff"); // redirect to maintenance dashboard
      } else {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-900 text-white">
      <h1 className="text-3xl font-bold mb-6">
        {isLogin ? "Maintenance Staff Login" : "Maintenance Staff Signup"}
      </h1>
